...
describe('Login', () => {
    beforeEach(() => {
        ...
        cy.visit("https://giveblockchain.io/save/")
        cy.url()
            .should("include", "/save/")
    })
    ...
    it('Expecting/Asserting all text on the page', () => {
        ...
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'First title text')
                .to.contain("Your contribution ensures that in addition to being able to rescue more girls, we can do more for the girls in each of our intervention strategies. Together we fight to end sexual exploitation of children!")
    ...
    it('Expecting/Asserting card clicks are working and accurate', () => {
        cy.get('[class="box-link"]')
            ...
            .click()
        cy.url()
            .should('include', "/save-25/")
        cy.go('back')
        cy.get('[class="box-link"]')
            .eq(1)
            .click()
        cy.url()
            .should('include', "/save-50/")
        cy.go('back')
        cy.get('[class="box-link"]')
            .eq(2)
            .click()
        cy.url()
            .should('include', '/save-100/')
        cy.go('back')
        cy.get('[class="box-link"]')
            .eq(3)
            .click()
        cy.url()
            .should('include', '/save-500/')
    })
})