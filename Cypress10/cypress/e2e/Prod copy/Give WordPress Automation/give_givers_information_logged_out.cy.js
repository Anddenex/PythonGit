...
describe('Login', () => {
    beforeEach(() => {
        cy.visit("https://giveblockchain.io/givers-information/")
        cy.url()
            .should("include", "/givers-information/")   
    })
    ...
    it('Expecting/Asserting all text on page', () => {
        ...
        cy.get('h2').should(($h2) => {
            ...
            expect($h2.eq(0), 'H2 Second Title').to.contain("THE GIVERS")
            expect($h2.eq(1), 'H2 Second Title').to.contain("Are you ready to")
        }) 
        ...
        cy.get('h4').should(($givers_title) => {
            ...
            expect($givers_title.eq(1), 'Second title text')
                .to.contain("Login or Sign up for Free to give to Givers.")
            expect($givers_title.eq(2), 'Third title text')
                .to.contain("FREE A GIRL")
            expect($givers_title.eq(3), 'Fourth title text')
                .to.contain("WHOLives")
            expect($givers_title.eq(4), 'Fifth title text')
    ...
    it('Expecting/Asserting for all button clicks that are active', () => {
        ...
        cy.get('h2').should(($h2) => {
            expect($h2.eq(0), 'H2 First Title').to.contain("Log in Now")
            expect($h2.eq(1), 'H2 Second Title').to.contain("Sign Up")
        })
        cy.go('back')
        cy.get('[class="nectar-button large regular has-icon regular-button"]')
            .eq(1)
