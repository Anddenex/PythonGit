...
describe('Login', () => {
    beforeEach(() => {
        cy.visit("https://giveblockchain.io/humanitarian/")
        cy.url()
    ...
    it('Expecting/Asserting all text on page', () => {
        cy.get('h2').should(($h2) => {
            expect($h2.eq(0), 'H3 First Title').to.contain('GIVE')
            expect($h2.eq(1), 'H3 First Title').to.contain('3')
            expect($h2.eq(2), 'H3 Second Title').to.contain('2')
            expect($h2.eq(3), 'H3 Second Title').to.contain('1')
            expect($h2.eq(4), 'H3 Second Title').to.contain('HOW WE SERVE')
            expect($h2.eq(5), 'H3 Second Title').to.contain('AWARDED SCHOLARSHIPS')
            expect($h2.eq(6), 'H3 Second Title').to.contain('Are you ready to')
        }) 
        ...
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'First title text')
                .to.contain("Your contribution helps fund our scholarship program for deserving individuals and families to experience one of our life-changing expeditions. Your donations may also be used for general overhead expenses and long-term humanitarian projects throughout the world.")
    ...
    it('Expecting/Asserting for all button clicks that are active', () => {
        cy.get('[class="nectar-button large regular has-icon regular-button"]')
            .eq(0)
            .click()
        cy.url().should('include', "/humanitarian-give/")
        cy.get('h3').should(($h3) => {
            expect($h3.eq(0), 'H2 First Title')
                .to.contain("Support Humanitarian")
        })
        cy.go('back')
        cy.get('[class="nectar-button large regular has-icon regular-button"]')
            .eq(1)
    ...
    it('Expecting/Asserting for all card clicks that are active', () => {
        cy.get('[class="box-link"]')
            .eq(0)
            .should('have.attr', 'href', '/humanitarian-25/')
            .click()
        cy.url()
            .should('include', '/humanitarian-25/')
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'H2 First Title')
                .to.contain("Humanitarian")
        })
        cy.go('back')
        cy.get('[class="box-link"]')
            .eq(1)
            .should('have.attr', 'href', '/humanitarian-50/')
            .click()
        cy.url()
            .should('include', '/humanitarian-50/')
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'H2 First Title')
                .to.contain("Humanitarian")
        })
        ...
        cy.get('[class="box-link"]')
            .eq(2)
            .should('have.attr', 'href', '/humanitarian-100/')
            .click()
        cy.url()
            .should('include', '/humanitarian-100/')
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'H2 First Title')
                .to.contain("Humanitarian")
        })
        ...
        cy.get('[class="box-link"]')
            .eq(3)
            .should('have.attr', 'href', '/humanitarian-500/')
            .click()
        cy.url()
            .should('include', '/humanitarian-500/')
        cy.get('h5').should(($h5) => {
            expect($h5.eq(0), 'H2 First Title')
                .to.contain("Humanitarian")
        })
