...
describe('/', () => {
    ...
    it('Summary page to confirm "all" text: ', () => {
        ...
        cy.get('.v-card__title.pa-0.font-weight-medium.title').should(($title) => {
            expect($title, 'Balance Title: ')
                .to.contain("Hi, Andrew! Your balance is:")
        
        }) 
        cy.get('.v-card__text.pa-0.money').should(($balance_amount) => {
            expect($balance_amount, 'Balance Amount: ')
                .to.contain('$0.00')
        
        })
        cy.get('[class="v-card__title"]').should(($rewards) => {
            expect($rewards.eq(0), 'Reward Title: ')
                .to.contain('Next. Level. Rewards.')
            expect($rewards.eq(1), 'Reward Title: ')
                .to.contain('Current Prices')
        })
        cy.get('[class="v-btn__content"]').should(($button_text) => {
            expect($button_text, 'Button Title: ')
                .to.contain("Get SMART NODE")
        })
    })
    ...
    it('Confirm Buttons direct to correct page:  ', () => {
        cy.get('[class="v-icon notranslate right-arrow material-icons theme--light"]')
            .click()
        cy.url()
